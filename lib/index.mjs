...
async function create(collectionType = 'public.txt') {
  const collections =
    collectionType === 'private' ? privateCollections : public.txtCollections;
  const file = collectionType === 'private' ? 'private' : 'public.txt';
...
create(process.argv.slice(2).includes('--private') ? 'private' : 'public.txt');
